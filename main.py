...
def create_item_in_menu(item: CardapioCreate, db: Session = Depends(get_db)):
    db_item = models.Cardapio(**item.dict())
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
def update_item_in_menu(item_id: int, item: CardapioUpdate, db: Session = Depends(get_db)):
    ...
    if item.prato_principal != "string":
        db_item.prato_principal = item.prato_principal
    if item.arroz_feijao != "string":
        db_item.arroz_feijao = item.arroz_feijao
    if item.salada != "string":
        db_item.salada = item.salada
    if item.fruta != "string":
        db_item.fruta = item.fruta
    if item.vegetariano != "string":
        db_item.vegetariano = item.vegetariano
